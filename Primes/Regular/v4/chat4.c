...
void generate_primes(mpz_t limit, char* filename) {
    mpz_t n, chunk_size, start, end, progress, index;
    mpz_inits(n, chunk_size, start, end, progress, index, NULL);
    ...
    mpz_set_ui(chunk_size, 1000000); // 1 million
    mpz_set_ui(start, 2);
    ...
    // Cleanup
    fclose(file);
    mpz_clears(n, chunk_size, start, end, progress, index, NULL);
